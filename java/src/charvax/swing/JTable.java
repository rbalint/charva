...
public class JTable
    ...
    public void processKeyEvent(KeyEvent ke_) {
        ...
        if (_model.getRowCount() == 0 || _model.getColumnCount() == 0) {
            term.beep();
            return;
        }
        ...
        if (key == KeyEvent.VK_UP) {
            ...
            if (_currentRow == _model.getRowCount() - 1)
                ...
                _currentRow += getStepSize();
                if (_currentRow > _model.getRowCount() - 1)
                    _currentRow = _model.getRowCount() - 1;
                int x = 0;
                for (int i = 0; i < _currentColumn; i++)
                    x += getColumnWidth(i) + 1;
    ...
    protected int getColumnWidth(int column_) {
    ...
    private void selectCurrentColumn() {
        if (_columnSelectionModel.isSelectedIndex(_currentColumn)) {
            _columnSelectionModel.removeSelectionInterval(_currentColumn, _currentColumn);
        } else {
            int selectionMode = _rowSelectionModel.getSelectionMode();

            // the column is not currently selected; select it.
            if (selectionMode == ListSelectionModel.SINGLE_SELECTION) {
                _columnSelectionModel.setSelectionInterval(_currentColumn, _currentColumn);
            } else {
                _columnSelectionModel.addSelectionInterval(_currentColumn, _currentColumn);
            }
        }
    }
    ...
    private void selectCurrentRow() {
        if (_rowSelectionModel.isSelectedIndex(_currentRow)) {
            _rowSelectionModel.removeSelectionInterval(_currentRow, _currentRow);
        } else {
            int selectionMode = _rowSelectionModel.getSelectionMode();

            // the row is not currently selected; select it.
            if (selectionMode == ListSelectionModel.SINGLE_SELECTION) {
                _rowSelectionModel.setSelectionInterval(_currentRow, _currentRow);
            } else {
                _rowSelectionModel.addSelectionInterval(_currentRow, _currentRow);
            }
        }
    }
